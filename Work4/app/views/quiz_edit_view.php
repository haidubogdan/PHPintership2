...
if (empty($edited_quiz_id)) {
    ...
    $current_edit_index="index.php?page=admin&operation=edit_quiz";
} else {
    $id = $edit_quiz->getQuizQuestionsIDsIdByID($edited_quiz_id);
    $added_questions = $id;
    $current_edit_index = "index.php?page=admin&operation=edit_quiz&quiz=".$edited_quiz_id;
}
...
<table>
    ...
    <?php foreach ($available_questions as $key => $row): ?>
        <tr><td style="min-width: 200px;">
            ...
            <td><a class="delete_button" href="<?= $current_index ?>">&#8595;</a></td>
            <td><a class="delete_button" href="<?= $current_edit_index . "&delete_quiz_question=" . $count ?>">X</a></td>
            <td><?= $key ?></td>
...
<form method="post" action="index.php?page=edit_quiz">
    <input type="text" name="quiz_id" value="<?= $quiz["id"] ?>" placeholder="quiz id" style='width:60px;'/>
    <input type="text" name="quiz_name" value="<?= $quiz["quiz_name"] ?>" placeholder="quiz name"/>
    <input type="text" name="description" value="<?= $quiz["description"] ?>" placeholder="description"/>
